 ...
 *
 * Props:
 *   brand : string — marque principale
 */
export default function PromptComparator({ brand }) {
  const [data, setData]       = useState(null);
  const [loading, setLoading] = useState(true);
  const [sortBy, setSortBy]   = useState('score'); // 'score' | 'mention_rate' | 'avg_position'
  const [filter, setFilter]   = useState('all');   // 'all' | 'mentioned' | 'absent'
